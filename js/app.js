  ...
  methods: {
    dateSplitter(date){
        if (date.length > 5) {
            ...
            let hour = dateArray[1].slice(0,-3) ; 
            return hour
        } else{
            return date
        }
    ...
    send(){
        ...
        if (this.newMessage != '') {
            let receiver = this.currentProfile;
            this.contacts[this.currentProfile].messages.push(newMessage)
            this.newMessage = '';
            this.autoReply(receiver)
    ...
    response(receiver){
        ...
        this.contacts[receiver].messages.push(newResponse)
